...
export function OutputPreferences() {
    ...
    // init predictionData as null if not using a preset
    ...
    return (
        <Container>
            ...
            <body className={`${styles.openSans}`} style={{fontSize:10, lineHeight:"300%", textAlign:'center'}}>Final steps...</body>
            <Display {...toggleMenuProps} />
            <ToggleMenu {...toggleMenuProps} />
            <VerticalSpace space="large" />
            <Button fullWidth disabled={predictionData==null} onClick={() => {
                const stylePreference = {
                    screenMode,
                    color,
                    fontSet
